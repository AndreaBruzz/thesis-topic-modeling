...
def main():
    ...
    run_id = "bert_chi_square_bert_emb_full"
    ...
    for _, query in queries.items():
        ...
        bert_output, _ = bert_helpers.run_bertopic_model(topwords, topics)
        ...
        for tid in top_topic_ids:
            top_words = [w for w, _ in bert_output.get_topic(tid)[:10]]
            print(f"  Topic {tid}: {' | '.join(top_words)}")

        documents_vectors = []
        for hit in hits:
            embedding = hit["_source"]["EMBEDDING_FULL"]
        ...
        embedded_topic_vectors = encoder.encode(topic_texts, normalize_embeddings=True)

        topic_vectors_named = [(f"t{i}", vec) for i, vec in enumerate(embedded_topic_vectors)]
        all_rerank_configs = []
        ...
        for run_name, topics_vectors in all_rerank_configs:
            ...
            utils.write_trec_run(query['num'], reranked, f'{run_name}', f'storage/runs/{run_id}')
